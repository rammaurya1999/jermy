    ...
    <div class="container-fluid p-0 ">
        <div class="row justify-content-between mb-3">
            <div class="col-md-6 text-start">
                <h3 class="">Manage Service Provider</h3>
            </div>
            <div class="col-md-6 text-end">
                {{-- <a href="{{ Route('admin.add-service-provider') }}" class="btn btn-primary text-light">Add Service
                    Provider
                </a> --}}
            </div>
        </div>
                                    ...
                                    <td class="tt">
                                        @if ($item->status == 1)
                                            <button class="btn btn-primary t"
                                                onclick="statusChange('{{ base64_encode('ServiceProvider') }}','{{ base64_encode($item->id) }}')">Active</button>
                                        @else
                                            <button class="btn btn-danger t"
                                                onclick="statusChange('{{ base64_encode('ServiceProvider') }}','{{ base64_encode($item->id) }}')">Inactive</button>
                                        @endif
                                    </td>
                                    <td>
                                        <a href="{{ Route('admin.view-service-provider', 'id=' . base64_encode($item->id)) }}"
                                            class="text-primary text-center"><i data-feather="eye"></i></a>
                                        || <a
                                            href="{{ Route('admin.delete', 'id=' . base64_encode($item->id) . '&modelName=' . base64_encode('ServiceProvider')) }}"
